> Parked reference: the pre-split monolithic backend/AGENTS.md from this fork (pre-upgrade-20260817). Upstream #4799 split agent guidance into per-directory AGENTS.md files with size budgets; ~250 fork-specific lines (mostly thread-boundary inventory tooling docs) were not carried into the split files. Kept here for reference until confirmed covered elsewhere.
